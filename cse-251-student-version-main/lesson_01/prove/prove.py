...
def run_with_threads(tur, log, main_turtle):
    ...
    t1 = threading.Thread(target = draw_circles(tur))
    t2 = threading.Thread(target = draw_squares(tur))
    t3 = threading.Thread(target = draw_rectangles(tur))
    t4 = threading.Thread(target = draw_triangles(tur))
